...
interface AIGovernanceReport {
  ...
  governance_label: 'Leading' | 'Developing' | 'Lagging' | 'At Risk';
  dimensions: {
    agent_visibility_governance: AIGovDimension;
    ai_absorbability: AIGovDimension;
    trust_infrastructure: AIGovDimension;
    decision_continuity: AIGovDimension;
  };
  ai_incident_debt: { level: 'High' | 'Moderate' | 'Low'; description: string };
  board_summary: string;
  top_governance_risks: Array<{ risk: string; severity: 'Critical' | 'High' | 'Moderate'; board_action: string }>;
...
const DIMENSION_LABELS: Record<string, string> = {
  agent_visibility_governance: 'Agent Visibility Governance™',
  ai_absorbability: 'AI Absorbability™',
  trust_infrastructure: 'Trust Infrastructure™',
  decision_continuity: 'Decision Continuity™',
};

function AIGovernanceResultPageInner() {
  ...
  useEffect(() => {
    if (!ticker || !sessionId) {
      router.replace('/reports/ai-governance');
      return;
    }
    generateReport();
  ...
  async function generateReport() {
    ...
    try {
      ...
      // Fetch FMP data
      let fmpData: { exchange?: string; sector?: string; marketCap?: number; priceChange3Y?: number } = {};
      try {
      ...
      const res = await fetch('/api/reports/ai-governance', {
        method: 'POST',
  ...
  async function exportPDF() {
    ...
    try {
      ...
      html2pdf()
        .set({
          margin: 10,
          filename: `ai-governance-${ticker.toLowerCase()}.pdf`,
          html2canvas: { scale: 2 },
    ...
    return (
      <main className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg font-semibold text-slate-700">Generating AI Governance Report™</p>
    ...
    <main className="min-h-screen bg-white">
      <div ref={reportRef}>
        ...
        <section className="section py-10 bg-slate-50">
          <div className="section-inner mx-auto max-w-3xl text-center">
            ...
            <div className={`text-8xl font-bold ${scoreColor(report.ai_governance_score)}`}>
              {report.ai_governance_score}
            </div>
            <p className="mt-2 text-xl font-semibold text-slate-700">{report.governance_label}</p>
            <div className="mt-4 flex justify-center gap-4 text-xs text-slate-400 flex-wrap">
              <span className="text-emerald-600 font-medium">80–100 Leading</span>
              <span className="text-amber-500 font-medium">60–79 Developing</span>
              <span className="text-orange-500 font-medium">40–59 Lagging</span>
              <span className="text-red-500 font-medium">0–39 At Risk</span>
            </div>
          </div>
        </section>

        {/* Board Summary — Premium Section */}
        <section className="section py-10">
          <div className="section-inner mx-auto max-w-3xl">
            <div className="rounded-2xl bg-slate-900 p-8">
              ...
              <p className="text-white leading-7 text-base">{report.board_summary}</p>
            </div>
          </div>
        </section>
        ...
        <section className="section py-10 bg-slate-50">
          <div className="section-inner mx-auto max-w-3xl">
            <h2 className="text-lg font-bold mb-6">Four Dimension Scores</h2>
            <div className="space-y-6">
              {Object.entries(report.dimensions).map(([key, dim]) => (
                <div key={key} className="card p-6">
                  ...
                  <p className="text-xs text-slate-400 italic">Risk: {dim.risk}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="section py-8">
          <div className="section-inner mx-auto max-w-3xl">
            <h2 className="text-lg font-bold mb-4">AI Incident Debt™</h2>
            <div className="card p-6">
              <span className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold mb-3 ${debtBadge(report.ai_incident_debt.level)}`}>
                {report.ai_incident_debt.level} Debt
              </span>
              <p className="text-sm text-slate-600 leading-6">{report.ai_incident_debt.description}</p>
            </div>
          </div>
        </section>
        ...
        <section className="section py-8 bg-slate-50">
          <div className="section-inner mx-auto max-w-3xl">
            <h2 className="text-lg font-bold mb-4">Top Governance Risks</h2>
            <div className="space-y-4">
              {report.top_governance_risks.map((item, i) => (
                <div key={i} className="card p-5">
                  <div className="flex items-start gap-3 mb-2">
                    <span className={`shrink-0 inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${severityBadge(item.severity)}`}>
                      {item.severity}
                    </span>
                    <p className="text-sm font-medium text-slate-800">{item.risk}</p>
                  </div>
                  <p className="text-xs text-slate-500 pl-0 italic">Board action: {item.board_action}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="section py-8 border-t border-slate-100">
        <div className="section-inner mx-auto max-w-3xl flex flex-wrap gap-3 justify-center">
          ...
          <Link
            href={`/enterprise/investor-stack/request?subject=Board AI Governance Briefing for ${encodeURIComponent(companyName)}`}
            className="inline-flex items-center gap-1.5 rounded-xl border border-orange-300 bg-orange-50 px-4 py-2 text-sm font-semibold text-orange-700 hover:bg-orange-100 transition-colors"
          >
          ...
          <button
            onClick={() => {
              if (navigator.share) {
                navigator.share({ title: `AI Governance Report™ — ${companyName}`, url: window.location.href });
              } else {
...
export default function AIGovernanceResultPage() {
  return (
